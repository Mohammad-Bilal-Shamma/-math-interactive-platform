  ...
  return (
    <section className="visualizer" aria-labelledby="visualizer-heading">
      ...
      <div className="visualizer__canvas">
        {kind === "interpolation" && <ResponsiveContainer width="100%" height={300}><LineChart key={kind} data={interpolationData}><CartesianGrid strokeDasharray="3 3" stroke="#e0e7ec" /><XAxis dataKey="x" /><YAxis /><Tooltip /><Legend /><Line type="monotone" dataKey="polynomial" stroke="#6f9fbb" strokeWidth={3} dot={false} name="كثيرة الحدود" isAnimationActive animationDuration={620} animationEasing="ease-out" /><Line type="linear" dataKey="points" stroke="#d88291" strokeWidth={0} dot={{ r: 5, fill: "#d88291" }} name="نقاط معطاة" isAnimationActive animationDuration={440} /></LineChart></ResponsiveContainer>}
        {kind === "integration" && <ResponsiveContainer width="100%" height={300}><AreaChart key={`${kind}-${segments}`} data={integrationData}><CartesianGrid strokeDasharray="3 3" stroke="#e0e7ec" /><XAxis dataKey="x" /><YAxis /><Tooltip /><Area type="monotone" dataKey="trapezoid" stroke="#6f9fbb" fill="#bfddec" fillOpacity={0.72} name="المساحة المقربة" isAnimationActive animationDuration={520} animationEasing="ease-out" /></AreaChart></ResponsiveContainer>}
        {kind === "newton" && <ResponsiveContainer width="100%" height={300}><LineChart key={`${kind}-${start}`} data={newtonData}><CartesianGrid strokeDasharray="3 3" stroke="#e0e7ec" /><XAxis dataKey="iteration" /><YAxis yAxisId="left" /><YAxis yAxisId="right" orientation="right" /><Tooltip /><Legend /><Line yAxisId="left" type="monotone" dataKey="x" stroke="#6f9fbb" strokeWidth={3} name="تقريب الجذر" isAnimationActive animationDuration={600} animationEasing="ease-out" /><Line yAxisId="right" type="monotone" dataKey="error" stroke="#d88291" strokeWidth={2} name="|f(x)|" isAnimationActive animationDuration={600} animationEasing="ease-out" /></LineChart></ResponsiveContainer>}
        {kind === "euler" && <ResponsiveContainer width="100%" height={300}><LineChart key={`${kind}-${step}`} data={eulerData}><CartesianGrid strokeDasharray="3 3" stroke="#e0e7ec" /><XAxis dataKey="x" /><YAxis /><Tooltip /><Legend /><Line type="stepAfter" dataKey="euler" stroke="#d88291" strokeWidth={3} name="أويلر" isAnimationActive animationDuration={540} animationEasing="ease-out" /><Line type="monotone" dataKey="exact" stroke="#6f9fbb" strokeWidth={3} dot={false} name="مرجع مقارن" isAnimationActive animationDuration={540} animationEasing="ease-out" /></LineChart></ResponsiveContainer>}
      </div>
